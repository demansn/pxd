...
The default registry (`defaultNodeTypes`) covers intrinsic types: `container`, `sprite`, `nineSliceSprite`, `text`, `graphics`, `slot`. Anything else in a doc — `"type": "SpinButton"`, `"type": "ProgressBar"`, `"type": "spine"` — needs a `NodeType` registered by the host.
